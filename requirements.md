...
- **FR-U6**: Get mentions for a user (`GET /users/:userId/mentions`) with optional `page`, `pageSize` → paginated comment list
...
- **FR-P8**: Get project workload (`GET /projects/:projectId/workload`) → array of `{ userId, username, openTicketCount }`
...
- **FR-T10**: `isOverdue` computed field — `true` if `dueDate` is in the past and status is not DONE
...
- **FR-C3**: Update comment (`PATCH /tickets/:ticketId/comments/:commentId`) with `content` → 200 OK, re-parses mentions
- **FR-C4**: Delete comment (`DELETE /tickets/:ticketId/comments/:commentId`) → 200 OK
- **FR-C5**: `@username` mentions in comment content are parsed, validated against existing users, and stored as M2M relation
...
- **FR-S1**: A background scheduler runs periodically (e.g. every hour)
- **FR-S2**: For each ticket where `dueDate` has passed and status is not DONE and priority is not CRITICAL: escalate priority to the next level (LOW→MEDIUM→HIGH→CRITICAL)
- **FR-S3**: Escalation is logged in the audit log with `actor: SYSTEM`

### 1.10 Auto-Assignment (Scheduler)
- **FR-S4**: For each ticket without an `assigneeId` in a project: assign to the DEVELOPER in that project with the fewest open (non-DONE) tickets
- **FR-S5**: If no DEVELOPER exists in the project, ticket remains unassigned
- **FR-S6**: Assignment is logged in the audit log with `actor: SYSTEM`
